...
// Add Person tab uses a two-step flow: check email → show appropriate form.

import { redirect } from "next/navigation";
import { requireAuth, pickActiveMembership, roleLabel, pbkdf2Hex, randomSaltHex } from "@/lib/auth";
...
async function updateRoleAction(formData: FormData) {
  ...
  if (!userId || !["STUDENT", "TEACHER", "SCHOOL_ADMIN"].includes(role)) redirect("/school-people");

  // Can't demote yourself from School Admin.
  if (userId === auth.user!.id && role !== "SCHOOL_ADMIN") {
    redirect("/school-people?error=self");
  }
...
async function removeMemberAction(formData: FormData) {
  ...
  // Remove from courses and teaching assignments.
...
async function checkEmailAction(formData: FormData) {
  ...
  if (u) {
    redirect(`/school-people?tab=add&email=${encodeURIComponent(email)}&exists=1&user_name=${encodeURIComponent(u.name)}`);
  }
  redirect(`/school-people?tab=add&email=${encodeURIComponent(email)}&exists=0`);
}
...
async function addNewUserAction(formData: FormData) {
  ...
  if (!name || !email || !["STUDENT", "TEACHER", "SCHOOL_ADMIN"].includes(role) || password.length < 6) {
    redirect("/school-people?tab=add&error=invalid");
  }
...
export default async function SchoolPeoplePage({
  ...
  searchParams: Promise<{
    tab?: string; role?: string; course_id?: string; class_id?: string;
    email?: string; exists?: string; user_name?: string; error?: string;
  }>;
}) {
  const auth = await requireAuth();
  ...
  return (
    <SchoolLayout auth={auth} active={active} currentPath="/school-people">
      ...
      {tab === "members" && <MembersTab tenantId={active.tenant_id} currentUserId={auth.user!.id} params={params} />}
      {tab === "add" && <AddPersonTab params={params} />}
    </SchoolLayout>
  );
}
...
async function MembersTab({
  tenantId, currentUserId, params,
}: {
  tenantId: string;
  currentUserId: string;
  params: { role?: string; course_id?: string; class_id?: string };
}) {
  const { all } = getDb();
  const filterRole = params.role || "";
  const filterCourseId = params.course_id || "";
  const filterClassId = params.class_id || "";
  ...
  // Batch context queries — courses and classes per member as pills.
  ...
  if (memberIds.length > 0) {
    ...
    const enrolledCourses = await all<{ user_id: string; title: string }>(
      `SELECT e.user_id, c.title FROM enrollments e JOIN courses c ON c.id=e.course_id
       WHERE e.user_id IN (${ph}) AND c.status='ACTIVE' ORDER BY c.title ASC`, memberIds
    );
    const taughtCourses = await all<{ user_id: string; title: string }>(
      `SELECT ct.user_id, c.title FROM course_teachers ct JOIN courses c ON c.id=ct.course_id
       WHERE ct.user_id IN (${ph}) AND c.status='ACTIVE' ORDER BY c.title ASC`, memberIds
    );
    const memberClasses = await all<{ user_id: string; name: string }>(
      `SELECT cs.user_id, cl.name FROM class_students cs JOIN classes cl ON cl.id=cs.class_id
       WHERE cs.user_id IN (${ph}) AND cl.status='ACTIVE' ORDER BY cl.name ASC`, memberIds
    );

    for (const r of enrolledCourses) {
      if (!coursesByUser[r.user_id]) coursesByUser[r.user_id] = [];
      coursesByUser[r.user_id].push(r.title);
    }
    for (const r of taughtCourses) {
      if (!coursesByUser[r.user_id]) coursesByUser[r.user_id] = [];
      if (!coursesByUser[r.user_id].includes(r.title)) coursesByUser[r.user_id].push(r.title);
    }
    for (const r of memberClasses) {
      if (!classesByUser[r.user_id]) classesByUser[r.user_id] = [];
      classesByUser[r.user_id].push(r.name);
    }
  }
  ...
  const allClasses = await all<{ id: string; name: string }>(
  ...
  return (
    <>
      {/* Filters */}
      <Card title="Filter">
        <form action="/school-people" method="get">
                    ...
                    <tr key={m.id} className="border-b border-gray-100">
                      <td className="py-3 px-2">
                        <div className="font-semibold">
                          {m.name}
                          {isSelf && (
                            <span className="ml-1 inline-block px-2 py-0.5 rounded-full bg-teal-50 text-teal-700 text-xs font-semibold">You</span>
                          )}
                        </div>
                        ...
                        {(userCourses.length > 0 || userClasses.length > 0) && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {userCourses.map((t, i) => (
                              <span key={`c${i}`} className="inline-block px-2 py-0.5 rounded-full bg-teal-50 text-teal-700 text-[11px] font-semibold">{t}</span>
                            ))}
                            {userClasses.map((n, i) => (
                              <span key={`cl${i}`} className="inline-block px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 text-[11px] font-semibold">{n}</span>
                            ))}
                          </div>
                        )}
                      </td>
...
// ============================================================
// Add Person Tab — two-step email check flow
// ============================================================

function AddPersonTab({
  params,
}: {
  params: { email?: string; exists?: string; user_name?: string; error?: string };
}) {
  const checkEmail = params.email || "";
  const exists = params.exists;
  const userName = params.user_name || "";

  // Step 1: No email checked yet — show email input.
  if (!checkEmail) {
    return (
  ...
  // Step 2a: User exists — just assign role.
  if (exists === "1") {
    return (
      <Card title="Add Existing User">
        <p className="text-sm mb-3">
          Found: <strong>{userName}</strong> ({checkEmail})
        </p>
        <p className="text-xs text-gray-500 mb-3">
          This person already has an account. Just choose their role at this school.
        </p>
        ...
        <p className="mt-3 text-xs">
          <a href="/school-people?tab=add" className="text-teal-700 hover:underline">Check a different email</a>
        </p>
      </Card>
    );
  }

  // Step 2b: New user — full creation form.
  return (
    <Card title="Create New User">
      <p className="text-xs text-gray-500 mb-3">
        No account found for <strong>{checkEmail}</strong>. Fill in the details to create one.
      </p>

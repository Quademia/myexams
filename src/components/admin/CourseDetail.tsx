...
// Course detail — 5 tabs (Details, Teachers, Students, Classes, Join Codes).
// Extracted from /school-course page for use inside the admin workspace.
...
const BASE = "/school?section=courses";
...
async function updateCourseAction(formData: FormData) {
  ...
  if (!courseId || !title) redirect(BASE);

  if (!["ACTIVE", "ARCHIVED"].includes(status)) redirect(`${BASE}&course_id=${courseId}&tab=details`);
  ...
  redirect(`${BASE}&course_id=${courseId}&tab=details&toast=Course+updated`);
}
...
async function assignTeacherAction(formData: FormData) {
  ...
  if (!courseId || !userId) redirect(BASE);
  ...
  if (!validMember) redirect(`${BASE}&course_id=${courseId}&tab=teachers`);

  const exists = await first("SELECT 1 FROM course_teachers WHERE course_id=? AND user_id=?", [courseId, userId]);
  if (!exists) {
    await run("INSERT INTO course_teachers (course_id, user_id, created_at) VALUES (?,?,?)", [courseId, userId, new Date().toISOString()]);
  }
  redirect(`${BASE}&course_id=${courseId}&tab=teachers&toast=Teacher+assigned`);
}
...
async function unassignTeacherAction(formData: FormData) {
  ...
  redirect(`${BASE}&course_id=${courseId}&tab=teachers&toast=Teacher+removed`);
}
...
async function enrolStudentAction(formData: FormData) {
  ...
  if (!validMember) redirect(`${BASE}&course_id=${courseId}&tab=students`);

  const now = new Date().toISOString();
  const exists = await first("SELECT 1 FROM enrollments WHERE course_id=? AND user_id=?", [courseId, userId]);
  if (!exists) {
    await run(
  ...
  redirect(`${BASE}&course_id=${courseId}&tab=students&toast=Student+enrolled`);
}
...
async function unenrolStudentAction(formData: FormData) {
  ...
  redirect(`${BASE}&course_id=${courseId}&tab=students&toast=Student+removed`);
}
...
async function enrolClassAction(formData: FormData) {
  ...
  if (!courseId || !classId) redirect(BASE);
  ...
  if (!classCheck || !courseCheck) redirect(`${BASE}&course_id=${courseId}&tab=classes`);
  ...
  redirect(`${BASE}&course_id=${courseId}&tab=classes&toast=Class+enrolled`);
}
...
async function unenrolClassAction(formData: FormData) {
  ...
  if (!cls) redirect(`${BASE}&course_id=${courseId}&tab=classes`);

  const classStudents = await all<{ user_id: string }>("SELECT user_id FROM class_students WHERE class_id=?", [classId]);
  for (const cs of classStudents) {
    await run("DELETE FROM enrollments WHERE course_id=? AND user_id=?", [courseId, cs.user_id]);
  }
  redirect(`${BASE}&course_id=${courseId}&tab=classes&toast=Class+removed`);
}
...
async function createCodeAction(formData: FormData) {
  ...
  else redirect(`${BASE}&course_id=${courseId}&tab=join-codes`);
  ...
  redirect(`${BASE}&course_id=${courseId}&tab=join-codes&toast=Join+code+created`);
}
...
async function revokeCodeAction(formData: FormData) {
  ...
  redirect(`${BASE}&course_id=${courseId}&tab=join-codes&toast=Join+code+revoked`);
}
...
export async function CourseDetail({ courseId, tab, tenantId, dupWho, dupAuto, dupMax }: CourseDetailProps) {
  ...
  if (newCode) cookieStore.delete("qa_new_code");

  const base = `${BASE}&course_id=${courseId}`;
  const tabs = [
  ...
  return (
    <>
      ...
      {tab === "details" && (
        <Card title="Edit Course">
          <form action={updateCourseAction}>
            ...
            <input name="title" defaultValue={course.title} required className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mb-3" />
            <label className="block text-sm mb-1">Status</label>
            <select name="status" defaultValue={course.status} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mb-3">
      ...
      {tab === "teachers" && <TeachersTab courseId={course.id} tenantId={tenantId} />}
      {tab === "students" && <StudentsTab courseId={course.id} tenantId={tenantId} />}
      {tab === "classes" && <ClassesTab courseId={course.id} tenantId={tenantId} />}
      {tab === "join-codes" && <JoinCodesTab courseId={course.id} tenantId={tenantId} newCode={newCode || undefined} dupWho={dupWho} dupAuto={dupAuto} dupMax={dupMax} />}
    </>
  );
}
...
async function TeachersTab({ courseId, tenantId }: { courseId: string; tenantId: string }) {
  ...
  const assigned = await all<{ id: string; name: string; email: string }>(
    `SELECT u.id, u.name, u.email FROM course_teachers ct JOIN qa_users u ON u.id = ct.user_id WHERE ct.course_id=? ORDER BY u.name ASC`,
    [courseId]
  );
  const allTeachers = await all<{ id: string; name: string; email: string }>(
    `SELECT u.id, u.name, u.email FROM memberships m JOIN qa_users u ON u.id = m.user_id WHERE m.tenant_id=? AND m.role IN ('TEACHER','SCHOOL_ADMIN') AND m.status='ACTIVE' AND u.status='ACTIVE' ORDER BY u.name ASC`,
    [tenantId]
  );
      ...
      {available.length > 0 && (
        <Card title="Assign Teacher">
          <form action={assignTeacherAction} className="flex gap-2 items-end">
            ...
            <button type="submit" className="px-4 py-2 bg-teal-700 text-white text-sm font-semibold rounded-lg hover:bg-teal-800">Assign</button>
          </form>
        </Card>
      )}
    </>
...
async function StudentsTab({ courseId, tenantId }: { courseId: string; tenantId: string }) {
  ...
  const enrolled = await all<{ id: string; name: string; email: string }>(
    `SELECT u.id, u.name, u.email FROM enrollments e JOIN qa_users u ON u.id = e.user_id WHERE e.course_id=? ORDER BY u.name ASC`,
    [courseId]
  );
      ...
      {available.length > 0 && (
        <Card title="Enrol Student">
          <form action={enrolStudentAction} className="flex gap-2 items-end">
            ...
            <button type="submit" className="px-4 py-2 bg-teal-700 text-white text-sm font-semibold rounded-lg hover:bg-teal-800">Enrol</button>
          </form>
        </Card>
      )}
    </>
...
async function ClassesTab({ courseId, tenantId }: { courseId: string; tenantId: string }) {
  ...
  const classes = await all<{ id: string; name: string; year_group: string | null; student_count: number }>(
    `SELECT cl.id, cl.name, cl.year_group, (SELECT COUNT(*) FROM class_students cs WHERE cs.class_id = cl.id) AS student_count FROM classes cl WHERE cl.tenant_id=? AND cl.status='ACTIVE' ORDER BY cl.name ASC`,
    [tenantId]
  );
  ...
  for (const cl of classes) {
    const enrolledFromClass = await all<{ user_id: string }>(
      `SELECT cs.user_id FROM class_students cs WHERE cs.class_id=? AND cs.user_id IN (SELECT e.user_id FROM enrollments e WHERE e.course_id=?)`,
      [cl.id, courseId]
    );
    classesWithEnrollment.push({ ...cl, enrolled_count: enrolledFromClass.length });
  }
  ...
  return (
    <>
            ...
            {linkedClasses.map((cl) => (
              <li key={cl.id} className="flex items-center justify-between text-sm">
                <span>
                  ...
                  <span className="text-gray-400 ml-2">— {cl.enrolled_count} of {cl.student_count} students enrolled</span>
                </span>
      ...
      {classesWithEnrollment.length > 0 ? (
        <Card title="Enrol Class Students in This Course">
          <p className="text-xs text-gray-500 mb-2">This will enrol all students in the selected class into this course. Students already enrolled are skipped.</p>
          <form action={enrolClassAction} className="flex gap-2 items-end">
            ...
            <select name="class_id" required className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm">
              {classesWithEnrollment.map((cl) => (<option key={cl.id} value={cl.id}>{cl.name} ({cl.student_count} students)</option>))}
            </select>
...
async function JoinCodesTab({ courseId, tenantId, newCode, dupWho, dupAuto, dupMax }: { courseId: string; tenantId: string; newCode?: string; dupWho?: string; dupAuto?: string; dupMax?: string }) {
  ...
  const codes = await all<{
    ...
    `SELECT jc.id, jc.scope, jc.role, c.title AS course_title, jc.auto_approve, jc.expires_at, jc.max_uses, jc.uses_approved
     FROM join_codes jc LEFT JOIN courses c ON c.id = jc.course_id
     WHERE jc.tenant_id=? AND jc.course_id=? AND jc.revoked=0 ORDER BY jc.created_at DESC`,
    [tenantId, courseId]
  );
    ...
    <>
              ...
              <tbody>
                {activeCodes.map((c) => (
                  <tr key={c.id} className="border-b border-gray-100">
                    ...
                    <td className="py-3 px-2 text-xs text-gray-500">Expires: {fmtISO(c.expires_at)}<br />Uses: {c.uses_approved}/{c.max_uses}</td>
                    <td className="py-3 px-2">
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${c.auto_approve === 1 ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}>
                    ...
                    <td className="py-3 px-2">
                      <div className="flex gap-1">
                        <a
                          href={`${BASE}&course_id=${courseId}&tab=join-codes&dup_who=${c.role === "STUDENT" ? "student" : "teacher"}&dup_auto=${c.auto_approve}&dup_max=${c.max_uses}#create`}
      ...
      <div id="create">
        <Card title={dupWho ? "Duplicate Code" : "Create Code for this Course"}>
          {dupWho && <p className="text-xs text-teal-700 mb-3">Pre-filled from an existing code. Adjust if needed, then click Create.</p>}
          <form action={createCodeAction}>
            ...
            <button type="submit" className="px-4 py-2 bg-teal-700 text-white text-sm font-semibold rounded-lg hover:bg-teal-800">Create code</button>
          </form>
        </Card>
      </div>
    </>

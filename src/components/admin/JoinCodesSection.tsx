// src/components/admin/JoinCodesSection.tsx
// Join Codes — Active Codes, Create Code, Pending Requests.
// Extracted from /school-join-codes page for use inside the admin workspace.
...
const BASE = "/school?section=join-codes";
...
async function createCodeAction(formData: FormData) {
  ...
  if ((scope! === "COURSE_ENROLL" || scope! === "COURSE_TEACHER") && course_id) {
    ...
    if (!c) redirect(`${BASE}&error=course_invalid`);
  }
  ...
  cookieStore.set("qa_new_code", codePlain, { path: "/school", httpOnly: true, secure: true, sameSite: "lax", maxAge: 60 });
  redirect(`${BASE}&toast=Join+code+created`);
}
...
async function revokeCodeAction(formData: FormData) {
  ...
  redirect(`${BASE}&toast=Join+code+revoked`);
}
...
async function approveRequestAction(formData: FormData) {
  ...
  const jc = await first<{
    ...
    `SELECT jc.* FROM join_codes jc WHERE jc.id=? AND jc.tenant_id=?`,
    [jr.join_code_id, active.tenant_id]
  );

  if (!jc) redirect(`${BASE}&error=code_invalid`);
  if (jc.revoked === 1) redirect(`${BASE}&error=code_revoked`);
  if (isIsoInPast(jc.expires_at)) redirect(`${BASE}&error=code_expired`);
  if (jc.uses_approved >= jc.max_uses) redirect(`${BASE}&error=code_maxed`);
  const code = jc!;
  ...
  if (Number(reserveResult?.meta?.changes || 0) === 0) {
    redirect(`${BASE}&error=code_invalid`);
  }
  ...
  try {
    ...
    if (curRole === "TEACHER" && code.role === "STUDENT" && (code.scope === "TENANT_ROLE" || code.scope === "COURSE_ENROLL")) {
      await unreserve();
      redirect(`${BASE}&error=teacher_demotion`);
    }
    ...
    if (code.scope === "COURSE_ENROLL" && code.course_id) {
      ...
      if (!courseCheck) { await unreserve(); redirect(`${BASE}&error=code_invalid`); }
      const roleToSet = curRole === "SCHOOL_ADMIN" ? "SCHOOL_ADMIN" : "STUDENT";
      if (!curRole) await ensureMembership(code.tenant_id, userId, roleToSet);
...
async function rejectRequestAction(formData: FormData) {
  ...
  redirect(`${BASE}&toast=Request+rejected&toast_type=error`);
}
...
interface JoinCodesSectionProps {
  tenantId: string;
  error?: string;
  dupWho?: string;
  dupAction?: string;
...
export async function JoinCodesSection({ tenantId, error, dupWho, dupAction, dupCourseId, dupAuto, dupMax }: JoinCodesSectionProps) {
  const cookieStore = await cookies();
  ...
  const [codes, courses, pending, history] = await Promise.all([
    all<{
      ...
      `SELECT jc.id, jc.scope, jc.role, jc.course_id, c.title AS course_title,
       ...
       WHERE jc.tenant_id=? ORDER BY jc.created_at DESC`, [tenantId]
    ),
    ...
    all<{
      ...
      `SELECT jr.id, jr.type, jr.requested_role, jr.created_at,
       ...
       WHERE jr.tenant_id=? AND jr.status='PENDING' ORDER BY jr.created_at DESC`, [tenantId]
    ),
    all<{
      ...
      `SELECT jr.id, u.name AS user_name, u.email AS user_email, jr.requested_role,
       ...
       ORDER BY jr.reviewed_at DESC LIMIT 50`, [tenantId]
    ),
  ]);
  ...
  const ERROR_MSGS: Record<string, string> = {
    no_course: "Please select a course when creating a course-specific code.",
    code_invalid: "Cannot approve: the join code was not found.",
    code_revoked: "Cannot approve: this join code has been revoked.",
    code_expired: "Cannot approve: this join code has expired.",
    code_maxed: "Cannot approve: this join code has reached its maximum uses.",
    course_invalid: "Course not found or inactive. Cannot create a code for it.",
    teacher_demotion: "Cannot approve: this user is already a Teacher and cannot be downgraded to Student via this code.",
    approve_failed: "Something went wrong while approving. The join code use was not consumed. Please try again.",
  };
                  ...
                  <tr key={c.id} className="border-b border-gray-100">
                    ...
                    <td className="py-3 px-2">
                      <div className="flex gap-1">
                        <a
                          href={`${BASE}&dup_who=${c.role === "STUDENT" ? "student" : "teacher"}&dup_action=${c.scope === "TENANT_ROLE" ? "school" : "course"}&dup_course_id=${c.course_id || ""}&dup_auto=${c.auto_approve}&dup_max=${c.max_uses}#create`}
                ...
                {pending.map((p) => (
                  <tr key={p.id} className="border-b border-gray-100">
                    ...
                    <td className="py-3 px-2 text-xs">
                      {p.course_title ? `${roleLabel(p.requested_role)} access to ${p.course_title}` : `${roleLabel(p.requested_role)} access to school`}
                    ...
                    <td className="py-3 px-2">
                      <div className="flex gap-1">
                        <form action={approveRequestAction}>
                          ...
                          <button type="submit" className="px-3 py-1 bg-teal-700 text-white text-xs font-semibold rounded-lg hover:bg-teal-800">Approve</button>
                        </form>
                  ...
                  {history.map((h) => (
                    <tr key={h.id} className="border-b border-gray-100">
                      ...
                      <td className="py-2 px-2 text-xs">
                        {h.course_title ? `${roleLabel(h.requested_role)} access to ${h.course_title}` : `${roleLabel(h.requested_role)} access to school`}
                      </td>
                      <td className="py-2 px-2">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${h.status === "APPROVED" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>{h.status}</span>
                        <div className="text-xs text-gray-400 mt-0.5">Reviewed: {fmtISO(h.reviewed_at)}</div>
                      </td>
                    </tr>
                  ))}
                </tbody>

// src/components/admin/SittingDetail.tsx
// Sitting builder — 3 tabs (Settings, Papers, Results).
// Extracted from /sitting-builder page for use inside the admin workspace.
...
async function saveSettingsAction(formData: FormData) {
  ...
  if (!sittingId || !title) redirect(SBASE);
  ...
  const { run } = getDb();
  await run(
    `UPDATE exam_sittings SET title=?, description=?, academic_year=?, status=?, updated_at=? WHERE id=? AND tenant_id=?`,
    [title, description, academicYear, status, new Date().toISOString(), sittingId, active.tenant_id]
  );
  redirect(`${SBASE}&sitting_id=${sittingId}&tab=settings&toast=Settings+saved`);
}
...
async function addExistingPaperAction(formData: FormData) {
  ...
  if (!already) {
    ...
    await run("INSERT INTO exam_sitting_papers (id, sitting_id, exam_id, sort_order, created_at) VALUES (?,?,?,?,?)",
      [crypto.randomUUID(), sittingId, examId, sortOrder, now]);
  }
  redirect(`${SBASE}&sitting_id=${sittingId}&tab=papers&toast=Paper+added`);
}
...
async function createPaperAction(formData: FormData) {
  ...
  if (!sittingId || !title || !courseId || !teacherId) {
    redirect(`${SBASE}&sitting_id=${sittingId}&tab=papers`);
  }
  ...
  if (!course) redirect(`${SBASE}&sitting_id=${sittingId}&tab=papers`);

  const teacher = await first("SELECT id FROM memberships WHERE user_id=? AND tenant_id=? AND role='TEACHER' AND status='ACTIVE'", [teacherId, active.tenant_id]);
  if (!teacher) redirect(`${SBASE}&sitting_id=${sittingId}&tab=papers`);

  await run(
    `INSERT INTO exams (id, tenant_id, course_id, title, status, created_by, created_at, updated_at) VALUES (?,?,?,?,'DRAFT',?,?,?)`,
    [examId, active.tenant_id, courseId, title, teacherId, now, now]
  );

  const alreadyTeaching = await first("SELECT course_id FROM course_teachers WHERE course_id=? AND user_id=?", [courseId, teacherId]);
  if (!alreadyTeaching) {
    await run("INSERT INTO course_teachers (course_id, user_id, created_at) VALUES (?,?,?)", [courseId, teacherId, now]);
  }
  ...
  redirect(`${SBASE}&sitting_id=${sittingId}&tab=papers&toast=Paper+created`);
}
...
async function removePaperAction(formData: FormData) {
  ...
  redirect(`${SBASE}&sitting_id=${sittingId}&tab=papers&toast=Paper+removed`);
}
...
interface SittingDetailProps {
  sittingId: string;
  tab: string;
  tenantId: string;
}

export async function SittingDetail({ sittingId, tab, tenantId }: SittingDetailProps) {
  const { first } = getDb();
  ...
  const base = `${SBASE}&sitting_id=${sittingId}`;
  const tabs = [
  ...
  return (
    <>
      <PageHeader title={sitting.title} subtitle={sitting.academic_year || "Sitting"} backHref={SBASE} backLabel="Back to Sittings" />
      ...
      {tab === "settings" && (
        <Card title="Settings">
          <form action={saveSettingsAction}>
            ...
            <button type="submit" className="px-4 py-2 bg-teal-700 text-white text-sm font-semibold rounded-lg hover:bg-teal-800">Save Settings</button>
          </form>
        </Card>
      )}

      {tab === "papers" && <PapersTab sittingId={sitting.id} tenantId={tenantId} />}
      {tab === "results" && <ResultsTab sittingId={sitting.id} tenantId={tenantId} />}
    </>
  );
}
...
async function PapersTab({ sittingId, tenantId }: { sittingId: string; tenantId: string }) {
  ...
  const papers = await all<{
    ...
    `SELECT esp.id AS paper_id, esp.exam_id, esp.sort_order,
     ...
     WHERE esp.sitting_id=? ORDER BY esp.sort_order ASC`,
    [sittingId]
  );
  ...
  if (papers.length > 0) {
    ...
    const gateCounts = await all<{ exam_id: string; gate_type: string; cnt: number }>(
      `SELECT exam_id, gate_type, COUNT(*) AS cnt FROM sitting_approval_gates WHERE tenant_id=? AND exam_id IN (${ph}) GROUP BY exam_id, gate_type`,
      [tenantId, ...examIds]
    );
  ...
  if (existingIds.length > 0) {
    ...
    availableExams = await all(
      `SELECT e.id, e.title, e.status, c.title AS course_title FROM exams e JOIN courses c ON c.id = e.course_id WHERE e.tenant_id=? AND e.id NOT IN (${notIn}) ORDER BY c.title ASC, e.title ASC`,
      [tenantId, ...existingIds]
    );
  } else {
    availableExams = await all(
      `SELECT e.id, e.title, e.status, c.title AS course_title FROM exams e JOIN courses c ON c.id = e.course_id WHERE e.tenant_id=? ORDER BY c.title ASC, e.title ASC`,
      [tenantId]
    );
  }

  const courses = await all<{ id: string; title: string }>("SELECT id, title FROM courses WHERE tenant_id=? AND status='ACTIVE' ORDER BY title ASC", [tenantId]);
  const teachers = await all<{ id: string; name: string }>(
    `SELECT DISTINCT u.id, u.name FROM memberships m JOIN qa_users u ON u.id = m.user_id WHERE m.tenant_id=? AND m.role='TEACHER' AND m.status='ACTIVE' ORDER BY u.name ASC`,
    [tenantId]
  );
                ...
                {papers.map((p, i) => {
                  ...
                  return (
                    <tr key={p.paper_id} className="border-b border-gray-100">
                      ...
                      <td className="py-3 px-2"><StatusBadge status={p.exam_status} /></td>
                      ...
                      <td className="py-3 px-2">
                        <div className="flex gap-1 flex-wrap">
                          <a href={`${SBASE}&sitting_id=${sittingId}&exam_id=${p.exam_id}`}
                            className="px-2 py-1 bg-gray-100 text-gray-700 text-xs font-semibold rounded-lg hover:bg-gray-200 no-underline">
                            Set Approvals
                          </a>
                          <a href={`${SBASE}&sitting_id=${sittingId}&exam_id=${p.exam_id}&edit=1`}
                            className="px-2 py-1 bg-gray-100 text-gray-700 text-xs font-semibold rounded-lg hover:bg-gray-200 no-underline">
                            Edit
                          </a>
          ...
          {availableExams.length > 0 ? (
            <form action={addExistingPaperAction}>
              ...
              <select name="exam_id" required className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mb-3">
                ...
                {availableExams.map((e) => (<option key={e.id} value={e.id}>{e.title} — {e.course_title} [{e.status}]</option>))}
              </select>
              <button type="submit" className="px-4 py-2 bg-teal-700 text-white text-sm font-semibold rounded-lg hover:bg-teal-800">Add Paper</button>
            </form>
          ) : (
        ...
        <Card title="Create New Paper">
          {courses.length > 0 ? (
            <form action={createPaperAction}>
              ...
              <button type="submit" className="px-4 py-2 bg-teal-700 text-white text-sm font-semibold rounded-lg hover:bg-teal-800">Create &amp; Add Paper</button>
            </form>
          ) : (
...
async function ResultsTab({ sittingId, tenantId }: { sittingId: string; tenantId: string }) {
  ...
  const paperList = await all<{ exam_id: string; exam_title: string; score_display: string | null; results_published_at: string | null }>(
    `SELECT esp.exam_id, e.title AS exam_title, e.score_display, e.results_published_at
     FROM exam_sitting_papers esp JOIN exams e ON e.id = esp.exam_id WHERE esp.sitting_id=? ORDER BY esp.sort_order ASC`,
    [sittingId]
  );

  if (paperList.length === 0) {
    return (<Card title="Results Overview"><p className="text-sm text-gray-400">No papers in this sitting yet. Add papers in the Papers tab.</p></Card>);
  }
  ...
  const studentList = await all<{ id: string; name: string }>(
    `SELECT DISTINCT u.id, u.name FROM exam_attempts ea JOIN qa_users u ON u.id = ea.user_id WHERE ea.exam_id IN (${ph}) AND ea.tenant_id=? AND ea.status='SUBMITTED' ORDER BY u.name ASC`,
    [...pIds, tenantId]
  );

  if (studentList.length === 0) {
    return (<Card title="Results Overview"><p className="text-sm text-gray-400">No submitted attempts found for any paper in this sitting.</p></Card>);
  }

  const attempts = await all<{
    id: string; exam_id: string; user_id: string; score_display: string | null;
    score_pct: number | null; score_raw: number | null; score_total: number | null;
    grade: string | null; pass_mark_percent: number | null;
  }>(
    `SELECT ea.id, ea.exam_id, ea.user_id, ea.score_display, ea.score_pct, ea.score_raw, ea.score_total, ea.grade, ea.pass_mark_percent
     FROM exam_attempts ea WHERE ea.exam_id IN (${ph}) AND ea.tenant_id=? AND ea.status='SUBMITTED' ORDER BY ea.attempt_no DESC`,
    [...pIds, tenantId]
  );
  ...
  function CellContent({ att, paper }: { att: typeof attempts[0] | undefined; paper: typeof paperList[0] }) {
    ...
    return (
      <div className="text-xs">
        ...
        {passed !== null && (
          <span className={`ml-1 font-bold ${passed ? "text-green-700" : "text-red-600"}`}>{passed ? "PASS" : "FAIL"}</span>
        )}
        <br />
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              {paperList.map((p) => (
                <th key={p.exam_id} className="text-center text-xs text-gray-500 uppercase py-2 px-2 max-w-[150px]">{p.exam_title}</th>
              ))}
            </tr>
          </thead>
          <tbody>
